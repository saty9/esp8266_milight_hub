...
std::string RF24PowerLevelHelpers::nameFromValue(const RF24PowerLevel& value) {
  const size_t ix = static_cast<size_t>(value);

  if (ix >= size(RF24_POWER_LEVEL_NAMES)) {
    printf("ERROR: unknown RF24 power level label - this is a bug!");
...
RF24PowerLevel RF24PowerLevelHelpers::valueFromName(const std::string& name) {
  ...
  printf("WARN: tried to fetch unknown RF24 power level: %s, using default.\n", name.c_str());
