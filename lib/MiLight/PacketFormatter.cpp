...
void PacketFormatter::pushPacket() {
  ...
  if ((currentPacket + packetLength) >= PACKET_BUFFER + PACKET_FORMATTER_BUFFER_SIZE) {
    printf("ERROR: packet buffer full!  Cannot buffer a new packet.  THIS IS A BUG!\n");
    return;
  }
...
void PacketFormatter::formatV1Packet(uint8_t const* packet, char* buffer) {
  buffer += sprintf(buffer, PSTR("Request type  : %02X\n"), packet[0]) ;
  buffer += sprintf(buffer, PSTR("Device ID     : %02X%02X\n"), packet[1], packet[2]);
  buffer += sprintf(buffer, PSTR("b1            : %02X\n"), packet[3]);
  buffer += sprintf(buffer, PSTR("b2            : %02X\n"), packet[4]);
  buffer += sprintf(buffer, PSTR("b3            : %02X\n"), packet[5]);
  buffer += sprintf(buffer, PSTR("Sequence Num. : %02X"), packet[6]);
}
